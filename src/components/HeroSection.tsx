...
const HeroSection = () => {
  ...
  // Auto-mute video when scrolling past hero, auto-unmute when coming back
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (entry.isIntersecting) {
          // User scrolled back to hero - unmute video
          v.muted = false;
          setMuted(false);
        } else {
          // User scrolled away from hero - mute video
          if (!v.muted) {
            v.muted = true;
            setMuted(true);
          }
          ...
          <div className="w-full max-w-7xl px-6 md:px-10">
            ...
            <FadeIn delay={0.85} y={20}>
              <p className="mt-5 md:mt-7 text-[10px] sm:text-xs md:text-sm font-medium uppercase tracking-[0.3em] text-white/75">
                 UI Designer · Developer · Prompt Engineer
              </p>
            </FadeIn>
          </div>
        </div>
